...
func (b *SuggestionPromptBuilder) Build(input SuggestionPromptInput) string {
	...
	writeLine(&sb, "- You may return multiple blocks (prefer 2-4 options when useful).")
	writeLine(&sb, "- Suggestions may cover upcoming days, not only today.")
	writeLine(&sb, "- If the user asks for best time/options, provide alternatives across different days or time windows.")
	writeLine(&sb, "- Do not include reminders, shopping lists or notes in blocks.")
	writeLine(&sb, "- Use type=event when proposing a fixed start/end time window.")
	writeLine(&sb, "- Use type=task for actions without fixed calendar slot.")
	writeLine(&sb, "- If the user asks for best study/workout/work session hours, prefer event with startsAt and endsAt.")
	writeLine(&sb, "- Use provided timezone and locale for temporal reasoning.")
	writeLine(&sb, "- For startsAt/endsAt, prefer explicit local offset (e.g. -03:00) instead of Z.")
	writeLine(&sb, "- If user explicitly asks for today, include at least one option today when feasible.")
